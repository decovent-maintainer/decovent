...
class Mouse(object):
    ...
    def on_click_01(self, x, y):
        assert decovent.default_context.active._Semaphore__value == 2
        sleep(1)
        self.point = Point(x, y)
        return self.point
    ...
    def on_click_02(self, x, y):
        assert decovent.default_context.active._Semaphore__value == 1
        sleep(.5)
        self.point = Point(x, y)
        return self.point
    ...
    def on_click_03(self, x, y):
        assert decovent.default_context.active._Semaphore__value == 0
        self.point = Point(x, y)
        return self.point
