...
class test_12(unittest.TestCase):
    ...
    def test(self):
        ...
        hash_ = str(hash(e_class)) + str(hash(e_meth)) + '_' \
            + e_meth.func_name
        self.assertEqual(hash_ in decovent.default_context.memos, True)

        self.assertEqual(len(h), 1)

        for (h_success, h_result, h_class, h_meth) in h:
            hash_ = str(hash(h_class)) + str(hash(h_meth)) + '_' \
                + h_meth.func_name

            if decovent.default_context.asynchronous is True:
